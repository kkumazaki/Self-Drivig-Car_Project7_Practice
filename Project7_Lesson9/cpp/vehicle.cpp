...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
  ...
  vector<string> states = Vehicle::successor_states();

  vector<Vehicle> trajectory;
  float cost;

  vector<Vehicle> best_trajectory = generate_trajectory("KL", predictions);
  float min_cost = 1.;

  if (states.size()==0){
      best_trajectory = generate_trajectory("KL", predictions);
  } else{
    for (int i = 0; i < states.size(); i++){     
        trajectory = generate_trajectory(states[i], predictions);
        cost = calculate_cost(*this, predictions, generate_trajectory(states[i], predictions));
        if (cost < min_cost){
            min_cost = cost;
